...
Aşama 10AA'da yerel kullanıcı hesabı e-posta ve tek yönlü bcrypt şifre özetiyle
PostgreSQL'e eklendi. Kayıt yalnız `USER` rolü üretir; başarılı kayıt/giriş
kimliği HttpOnly sunucu oturumuna yazar. `CurrentActorProvider` aynı kaldığı için
gameplay, XP ve leaderboard kimlik doğrulama ayrıntılarına bağlanmaz. Geçici
header adapter'ı yalnız local/test admin ve otomasyon akışları için korunur.
Production OIDC ve CSRF sözleşmesi bu yerel hesap aşamasının dışındadır.
...
Aşama 10D'de quiz yerleşimi `CONTENT`, `SEASON` veya `EPISODE` olarak açıkça
modellendi. Quiz modülü content tablolarını sorgulamaz; aidiyeti content
modülünün `ContentReferenceVerifier` portundan doğrular. Flyway V10 check ve
bileşik foreign key constraint'leri application kuralını kalıcı veri sınırında
da korur. Eski kayıtlar `CONTENT` kapsamına taşınır.
...
Admin medya listesi yalnız immutable metadata'yı sayfalı döndürür ve geriye
uyumluluk için korunur. Güncel admin web bu listeyi kütüphane olarak göstermez;
kapak ve soru görselleri ilgili formdan doğrudan yüklenir. Binary içerik mevcut
kimlik doğrulamalı content endpoint'inden ayrıca okunur ve bağlı medya kimlikleri
değişmez kalır.
...
XP işlem defteri ve kullanıcı XP özetini yönetir. Gameplay,
`FIRST_COMPLETION_SCORE_V2` ile `(userId, quizId)` başına ilk tamamlamayı tek
ödül hakkı olarak belirler. İlk attempt'in skoru XP'ye çevrilir; sonraki attempt
skorları ledger kaydı üretmez. `QUIZ_COMPLETED` kaydı kaynak attempt ve
deterministik referansla tekildir. Düzeltmeler geçmiş satırı değiştirmez, özgün kazanca bağlı
...
Inbox deduplication'ı ve RabbitMQ adapter'larını yönetir. `quiz.completed` v2
olayı attempt kimliğinden deterministik event ID üretir ve ilk-tamamlama ödül
hakkını açıkça taşır; consumer v1 olaylarını da okumayı sürdürür. Publisher confirm
...
10D ile content yönetimine quiz yazarlığı ve medya yeniden kullanımı eklendi.
11.08.2026 bakım değişikliğinde gömülü görsel kütüphanesi arayüzden kaldırıldı;
kapak ve soru formları doğrudan dosya yüklemeye devam eder. Frontend
content, media ve quiz admin sözleşmelerini çağırır; domain kuralını veya
authorization kararını tekrar uygulayan otorite değildir. Kapalı soru
editörleri gecikmeli render edilir; böylece her soru için gereksiz medya
önizleme istekleri üretilmez.
...
kanıtlandıktan sonra Aşama 6'da `quiz.completed` integration event'inin XP
...
yanıtındaki `earnedXp` kalıcı ilk-tamamlama hakkıyla hemen kesinleşir; XP özeti consumer çalışana
...
- `FIRST_COMPLETION_SCORE_V2`, kullanıcı ve quiz başına yalnız ilk tamamlamanın
  kesin sunucu skorunu XP yapar; sonraki attempt'ler `0 XP` alıştırmadır.
...
- En iyi attempt seçilmez; ilk-tamamlama hakkıyla ledger'a giren completion XP'leri ve onların
