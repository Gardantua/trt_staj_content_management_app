...
Yazar: Yunus Emre Arı (yunuseemreari@gmail.com)  
Kurum: Kocaeli Üniversitesi, Bilgisayar Mühendisliği Bölümü  
Bağlam: TRT Staj Projesi - İçerik Etkileşim ve Oyunlaştırma Platformu  
Canlı Yayın Alan Adı: https://hikayeizi.duckdns.org  

## İçindekiler
1. [Özetçe ve Abstract](#özetçe-ve-abstract)
2. [I. Giriş](#i-giriş)
3. [II. Sistem Gereksinimleri ve Tasarım Hedefleri](#ii-sistem-gereksinimleri-ve-tasarım-hedefleri)
4. [III. Sistem Mimarisi ve Modül Sınırları](#iii-sistem-mimarisi-ve-modül-sınırları)
5. [IV. Domain ve Veritabanı Tasarımı](#iv-domain-ve-veritabanı-tasarımı)
6. [V. Kritik Sistem Akışları ve Mesajlaşma](#v-kritik-sistem-akışları-ve-mesajlaşma)
7. [VI. Güvenlik, Erişilebilirlik ve Dayanıklılık](#vi-güvenlik-erişilebilirlik-ve-dayanıklılık)
8. [VII. Uygulama ve Canlı Dağıtım Topolojisi](#vii-uygulama-ve-canlı-dağıtım-topolojisi)
9. [VIII. Test ve Doğrulama Metodolojisi](#viii-test-ve-doğrulama-metodolojisi)
10. [IX. Karşılaşılan Sorunlar ve Mühendislik Çözümleri](#ix-karşılaşılan-sorunlar-ve-mühendislik-çözümleri)
11. [X. Tartışma ve Trade-off Değerlendirmesi](#x-tartışma-ve-trade-off-değerlendirmesi)
12. [XI. Sonuç ve Gelecek Çalışmalar](#xi-sonuç-ve-gelecek-çalışmalar)
13. [Bilgilendirme ve Kaynaklar](#bilgilendirme-ve-kaynaklar)
...
Çekirdek uygulama on mantıksal modüle ayrılmıştır:
- identity: Kullanıcı kaydı, parola özetleme ve şifre sıfırlama işlemlerini yürütür (identity_user_accounts, password_reset_tokens).
- content: Dizi, film, sezon ve bölüm kataloğunu yönetir (content_catalog_items, content_seasons, content_episodes).
- media: Görsel dosyaların depolanmasını ve SHA-256 bütünlük kontrolünü sağlar (media_assets).
- quiz: Soruların, seçeneklerin ve sürümlerin yaşam döngüsünden sorumludur (quiz_definitions, quiz_versions, quiz_questions, question_options).
- gameplay: Attempt başlatma, deadline üretimi, cevap doğrulama ve skorlamayı yönetir (gameplay_attempts, attempt_answers, gameplay_quiz_reward_claims).
- gamification: Append-only XP işlem defterini tutar (xp_transactions).
- messaging: Transactional Outbox ve Inbox altyapısını işletir (outbox_events, inbox_messages).
- leaderboard: Dış mikroservis ile entegrasyonu sağlar.
- admin: Yönetim yetkilerini koordine eder.
- shared: Ortak hata ve izleme yapılarını barındırır.

Modüller birbirlerinin tablolarına doğrudan erişemez; iletişim yalnızca servis arayüzleri ve sözleşmeler üzerinden yürütülür.
...
## D. Leaderboard Servisinin Ayrıştırılması (ADR-0031)
...
Quiz tamamlandığında attempt durumunun güncellenmesi ile tamamlanma olayının oluşturulması aynı yerel veritabanı transaction'ı içinde atomik olarak gerçekleştirilir. Böylece ağ kesintilerinde dahi veri kaybı önlenir. Arka plan yayıncı servisi bekleyen olayları toplu olarak çeker, RabbitMQ kuyruğuna iletir ve onay aldıktan sonra durumu günceller. Tüketici servis mesajı aldığında Inbox tablosunu kontrol eder; ilk kez gelen olay için XP defterine kayıt atar ve yeni bir XP değişim olayını Outbox'a bırakır. Tekrar iletilen mesajlar Inbox kalkanı sayesinde yok sayılır.
...
# VII. UYGULAMA VE CANLI DAĞITIM TOPOLOJİSİ
...
Sistem Oracle Cloud Always Free sanal sunucusu üzerinde sekiz izole Docker konteyneri halinde canlıya alınmıştır. İnternete yalnızca Port 80 ve Port 443 açılmıştır. Veritabanları, mesaj brokerı ve servisler dış dünyaya kapalı bir dahili köprü ağı üzerinden haberleşir. Tüm veriler Docker Named Volume disklerinde saklanır.
...
# VIII. TEST VE DOĞRULAMA METODOLOJİSİ
...
## Yerel Çalıştırma ve Doğrulama Komutları
