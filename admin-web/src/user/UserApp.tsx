...
function ContentDetail({ api, contentId, onBack, onStartQuiz }: { api: PublicApi; contentId: string; onBack: () => void; onStartQuiz: (quizId: string) => void }) {
  ...
  return <><section className="detail-hero-viewer"><Cover api={api} content={content} prominent /><div><button className="text-button light" onClick={onBack}>← {t("viewer.backToDiscover")}</button><p className="kicker">{t(content.contentType === "SERIES" ? "common.series" : "common.film")}</p><h1>{content.title}</h1><p>{content.description || t("viewer.contentDescriptionSoon")}</p><OfficialWatchLink watchUrl={content.watchUrl} label={t("viewer.watchOnTabii")} /></div></section><section className="detail-section"><ErrorNotice error={error} /><QuizShelf content={content} quizzes={quizzes} quizResults={quizResults} language={language} onStartQuiz={onStartQuiz} /></section></>;
}
